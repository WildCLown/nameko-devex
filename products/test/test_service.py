...
def test_delete_product(create_product, service_container):
    ...
    with entrypoint_hook(service_container, 'delete_one') as delete:
        loaded_product = delete(stored_product['id'])
    
    assert loaded_product == {"deleted_id": stored_product['id']}

def test_delete_product_on_not_found(create_product, service_container):
